...
//###INSERT CODE HERE -
    SinglyLinkedListNode* a = head_list1;
    SinglyLinkedListNode* b = head_list2;
    multiset<int> arr;
    while(a){
        arr.insert(a->data);
        a = a->next;
    ...
    SinglyLinkedList* finall = new SinglyLinkedList();

    for (int x : arr){
        SinglyLinkedListNode* newnode = new SinglyLinkedListNode(x);
        if (finall->head == nullptr)
            finall->head = newnode;
        else
            finall->tail->next = newnode;
        finall->tail = newnode;
    }
    return finall->head;
}
